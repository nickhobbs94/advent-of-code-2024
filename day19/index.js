...
function combos(pattern) {
  if (pattern === '') return 1;
  if (memos[pattern] !== undefined) return memos[pattern];

  console.log("combos", pattern);

  let count = 0;
  for (let towel of towels) {
    if (pattern.startsWith(towel)) {
      count += combos(pattern.slice(towel.length))
    }
  }
...
console.log(patterns.map(combos).reduce((acc,e) => acc+e, 0));
